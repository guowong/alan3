...
public class BooksAndClothesPolymorphism {
    //alan private static Item[] shoppingCart = new Item[100];   // contains items in user's shopping cart
	private static int USE_LENGTH = 10;  //alan added
	private static Item[] shoppingCart = new Item[USE_LENGTH];  //alan added
    ...
    private static final Item[] ITEM_LIST = {
        ...
        new Shirt(2868,"Dress shirt", "Ralph Lauren", "L", "White", 39.95, 2),
        ...
        do {
            try {
                ...
                for (itemIndex=0; itemIndex<ITEM_LIST.length; itemIndex++)
                    ...
                    {
                    	nCurInStock = ITEM_LIST[itemIndex].getInStock();                  //alan added
                    	//System.out.println(nCurInStock);                                  //alan added
                    	if (nCurInStock ==  0)                                            //alan added
                    		System.out.println("Out of stock, Please try again later");   //alan added
                    	else                                                              //alan added
                    		ITEM_LIST[itemIndex].setInStock(nCurInStock - 1);             //alan added
                ...
                if (itemIndex == ITEM_LIST.length)  // reached the end and not found
                	...
                	//System.out.println(shoppingCartCount);
                	if (shoppingCartCount < USE_LENGTH) {   //Alan added
                        shoppingCart[shoppingCartCount] = ITEM_LIST[itemIndex];
                        shoppingCartCount++;  // keep track of items in the cart
                	} else {   //alan added
                		System.out.println("Your shopping cart is full. Please check out.");  //alan added
                	}  //alan added
                }
            }
